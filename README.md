
<h1 align="center">Hi 👋, I'm Nelluri Dolendra Sai Teja</h1>

<h3 align="center">
🚀 Full Stack Developer | 🤖 Machine Learning Enthusiast | ⚡ DevOps Learner | 🧠 Generative AI Explorer
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&pause=1000&color=00F7FF&center=true&vCenter=true&width=900&lines=Generative+AI;Machine+Learning+Engineer;MERN+Stack+Developer;DevOps+Explorer;Building+Real+World+Projects;Always+Learning+New+Things" />
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Dolendra&label=Profile%20Views&color=0e75b6&style=for-the-badge" />
</p>

---

<img align="right" alt="Coding" width="400" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif">


# 💫 About Me

🎓 B.Tech Computer Science and Engineering Student at **RGUKT Basar**

💻 Passionate about:

- Full Stack Development
- Machine Learning
- Deep Learning
- Generative AI
- DevOps
- System Design
- Problem Solving

🚀 Currently Building:

- AI-Powered Full Stack Applications
- Scalable MERN Stack Projects
- Intelligent Automation Systems
- Real-World AI Solutions

📚 Currently Learning:

- Generative AI & LLM Applications
- DevOps & Cloud Technologies
- Advanced Backend Architecture
- ML Model Deployment
- Scalable System Design

✨ Fun Fact:

> I love turning ideas into real-world AI-powered applications.

---

# 🌐 Connect With Me

<p align="left">

<a href="https://github.com/Dolendra" target="_blank">
<img src="https://skillicons.dev/icons?i=github" />
</a>

<a href="https://linkedin.com/in/dolendranelluri" target="_blank">
<img src="https://skillicons.dev/icons?i=linkedin" />
</a>

<a href="https://instagram.com/dolendra_n13" target="_blank">
<img src="https://skillicons.dev/icons?i=instagram" />
</a>

<a href="mailto:dolendracse@gmail.com">
<img src="https://skillicons.dev/icons?i=gmail" />
</a>

</p>

---

# 💻 Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=java,python,c,cpp,javascript,typescript,react,nodejs,express,mongodb,mysql,html,css,tailwind,bootstrap,git,github,docker,linux,flask,django,tensorflow,pytorch,vscode,postman" />

</p>
<p align="center"> <img src="https://img.shields.io/badge/LangChain-121D33?style=for-the-badge&logo=chainlink&logoColor=white"/> <img src="https://img.shields.io/badge/Pinecone-000000?style=for-the-badge&logo=pinecone&logoColor=white"/> <img src="https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white"/> <img src="https://img.shields.io/badge/Groq-F55036?style=for-the-badge&logo=groq&logoColor=white"/> <img src="https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black"/> <img src="https://img.shields.io/badge/ScikitLearn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/> <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/> <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/> <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white"/> <img src="https://img.shields.io/badge/Mediapipe-0097A7?style=for-the-badge&logo=google&logoColor=white"/> <img src="https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white"/> <img src="https://img.shields.io/badge/GoogleColab-F9AB00?style=for-the-badge&logo=googlecolab&logoColor=black"/> </p>

---

# 🚀 Featured Projects

---

# 💻 Software Development Projects

## 🔹 Agri-Sphere
> Smart agriculture platform designed to help farmers with modern digital solutions.

### Features:
- User Authentication
- Crop Information System
- Farmer-Friendly Dashboard
- Responsive Frontend
- Backend API Integration

### Tech Stack:
`React` `Node.js` `Express.js` `MongoDB`

---

## 🔹 Stitching with Love — E-Commerce Website
> Full Stack MERN E-Commerce platform for clothing and custom stitching services.

### Features:
- JWT Authentication
- Product Listings
- Shopping Cart
- Order Management
- Admin Dashboard
- Responsive UI

### Tech Stack:
`React` `Node.js` `Express.js` `MongoDB` `Tailwind CSS`

---

# 🤖 Machine Learning Projects

## 🔹 Car Price Prediction
> Machine Learning model that predicts car prices based on various features.

### Features:
- Data Preprocessing
- Feature Engineering
- Model Training
- Regression Analysis
- Performance Evaluation

### Tech Stack:
`Python` `Scikit-Learn` `Pandas` `NumPy` `Matplotlib`

---

## 🔹 Email Spam Detection
> Intelligent spam classifier using Natural Language Processing techniques.

### Features:
- Text Cleaning
- NLP Processing
- TF-IDF Vectorization
- Classification Models
- Spam Detection System

### Tech Stack:
`Python` `Scikit-Learn` `NLP` `Pandas`

---

# 🧠 Deep Learning Projects

## 🔹 Face Recognition and Attendance Manager
> AI-powered attendance management system using face recognition.

### Features:
- Real-Time Face Detection
- Attendance Automation
- Face Encoding
- Student Record Management
- Live Camera Integration

### Tech Stack:
`Python` `OpenCV` `Flask` `Mediapipe`

---

## 🔹 AI Proctoring System
> Intelligent online exam monitoring system using computer vision.

### Features:
- Face Tracking
- Eye Movement Detection
- Suspicious Activity Monitoring
- Real-Time Alerts
- AI-Based Proctoring

### Tech Stack:
`Python` `OpenCV` `Deep Learning`

---

# ⚡ Generative AI Projects

## 🔹 AI-Powered Market Trend and Consumer Sentiment Forecaster
> GenAI system for analyzing market trends and customer sentiment.

### Features:
- Sentiment Analysis
- Embedding Generation
- Vector Database Integration
- Trend Forecasting
- AI-Powered Insights

### Tech Stack:
`Python` `LangChain` `Pinecone` `LLMs` `Groq/OpenAI`

---

## 🔹 College ChatBot
> AI chatbot developed for assisting students with college-related information.

### Features:
- Natural Language Interaction
- Student Query Handling
- AI-Based Responses
- Real-Time Assistance

### Tech Stack:
`Python` `LLMs` `LangChain` `Flask`

---

---

# 📊 GitHub Stats

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Dolendra&show_icons=true&theme=tokyonight&hide_border=true" height="190"/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Dolendra&theme=tokyonight&hide_border=true" height="190"/>

</p>

---

# 📈 Contribution Graph

<p align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Dolendra&theme=tokyo-night&hide_border=true" width="100%"/>
</p>

---

# 🏆 GitHub Achievements

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Dolendra&theme=radical&no-frame=true&no-bg=true&margin-w=4&row=1"/>
</p>

---

# 💻 Coding Profiles

<p align="center">

<a href="https://leetcode.com/YOUR_LEETCODE/">
<img src="https://img.shields.io/badge/LeetCode-000?style=for-the-badge&logo=leetcode&logoColor=yellow"/>
</a>

<a href="https://www.codechef.com/users/YOUR_CODECHEF">
<img src="https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge&logo=codechef&logoColor=white"/>
</a>

<a href="https://www.hackerrank.com/YOUR_HACKERRANK">
<img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=HackerRank&logoColor=white"/>
</a>

</p>

---

# ⚡ GitHub Metrics

<p align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Dolendra&theme=tokyonight"/>

</p>

---

# 🔥 Current Focus

```txt
Building Scalable Full Stack Applications
Learning DevOps & Cloud Technologies
Improving DSA & System Design
Exploring AI + Full Stack Integration
````

---

# ✨ Quote

### "Consistency + Curiosity = Great Developer"

---
